...
export function MaxRoom(props: { memoryCount: number; activity?: RoomActivity }): ReactElement {
  ...
  const sceneNotes: Record<RoomActivity, string> = { idle: room.sceneNote, thinking: 'Max 正在把你的话放在心上。', speaking: 'Max 有话想认真回应你。' };
  return (
    <section className={`room-card room-${room.timeOfDay}`} aria-label="Max 的房间">
      ...
      <div className={`room-scene scene-${activity}`}>
        <div className="room-backdrop" data-testid="room-backdrop" aria-hidden="true">
          <div className="room-wall-shelf"><span /><span /><span /></div>
          <div className="room-window-view"><i /><i /><i /></div>
